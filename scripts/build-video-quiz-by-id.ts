/**
 * Construit src/data/video-quiz-by-id.ts avec des questions spécifiques par vidéo.
 * Exécuter : npx tsx scripts/build-video-quiz-by-id.ts
 */
import { writeFileSync } from "fs";
import { JW_VIDEOS } from "../src/data/jw-videos";
...
/** Questions spécifiques — complément à video-quiz-bank.ts */
const SPECIFIC: Record<string, QData[]> = {
  // ── GNJ : 1 question extra (banque couvre déjà 2-3) ──
  "gnj-pisode-1-la-vraie-lumi-re-du-monde": [
    {
      ...
      options: [
        ...
        "« Retournons à la pêche »",
        "« Jésus est un simple rabbin »",
      ],
      correctIndex: 0,
  ...
  "gnj-pisode-3-je-suis-celui-ci": [
    {
      ...
      options: [
        "« L'esprit de Jéhovah est sur moi… pour prêcher la bonne nouvelle aux pauvres »",
  ...
  "gnj-pisode-4-c-est-pour-cela-que-je-suis-venu": [
    {
      ...
      options: [
        "Parce qu'il est ému de compassion pour elle",
        "Pour recevoir de l'argent",
        "Pour devenir roi de la ville",
        "Parce qu'on le lui a ordonné par la force",
      ],
      correctIndex: 0,
  ...
  "gnj-pisode-5-bahis-par-sa-mani-re-d-enseigner": [
    {
      ...
      options: [
        "Jésus enseigne avec autorité, pas comme les scribes",
        "Jésus ne cite jamais les Écritures",
        "Les scribes enseignent mieux que Jésus",
        "Jésus enseigne seulement en grec",
      ],
      correctIndex: 0,
  ...
  "gnj-son-nom-est-jean": [
    {
      ...
      options: [
        "Parce que l'ange de Jéhovah l'avait ordonné",
        "Parce que c'était la mode à l'époque",
        "Parce que les voisins l'ont choisi",
        "Parce que Jean signifie « roi »",
      ],
      correctIndex: 0,
...
// Continue in part 2 — append remaining videos
const SPECIFIC_PART2: Record<string, QData[]> = {
  // ── Chansons enfants ──
...
Object.assign(SPECIFIC, SPECIFIC_PART2);

const SPECIFIC_PART3: Record<string, QData[]> = {
  // ── Ados spiritualité ──
...
Object.assign(SPECIFIC, SPECIFIC_PART3);

const SPECIFIC_PART4: Record<string, QData[]> = {
  // ── Ados social ──
...
Object.assign(SPECIFIC, SPECIFIC_PART4);

const SPECIFIC_PART5: Record<string, QData[]> = {
  // ── Famille ──
...
Object.assign(SPECIFIC, SPECIFIC_PART5);

const SPECIFIC_PART6: Record<string, QData[]> = {
  // ── Chansons pkon ──
  "pkon-001": [
    {
...
Object.assign(SPECIFIC, SPECIFIC_PART6);

function formatQuestion(videoId: string, index: number, data: QData): string {
  const id = `${videoId}-s${index + 1}`;
  const opts = data.options.map((o) => JSON.stringify(o)).join(", ");
  return `    q(${JSON.stringify(id)}, {
...
for (const video of JW_VIDEOS) {
  const questions = SPECIFIC[video.id];
  if (!questions || questions.length === 0) {
    missing.push(video.id);
    continue;
  }
  const qs = questions.map((q, i) => formatQuestion(video.id, i, q)).join(",\n");
  entries.push(`  ${JSON.stringify(video.id)}: [\n${qs},\n  ]`);
}

const file = `/** Questions spécifiques par vidéo — complément à video-quiz-bank.ts */
...
writeFileSync("src/data/video-quiz-by-id.ts", file);

const covered = JW_VIDEOS.length - missing.length;
console.log(`Wrote ${covered}/${JW_VIDEOS.length} video quiz sets to src/data/video-quiz-by-id.ts`);
if (missing.length) {
