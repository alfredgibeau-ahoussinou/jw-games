  ...
  return (
    <article className="overflow-hidden rounded-xl border border-white/[0.06] bg-[var(--bg-card)]">
      ...
      >
        ...
        <ChevronDown
          className={cn(
            "mt-0.5 h-5 w-5 shrink-0 text-[var(--text-dim)] transition-transform",
            open && "rotate-180"
          )}
          aria-hidden
        />
      ...
      {open && (
        <div className="space-y-3 border-t border-white/[0.05] px-4 pb-4 pt-3 sm:px-5 sm:pb-5">
          {reading.paragraphs.map((p, i) => (
            <p key={i} className="text-[0.9375rem] leading-relaxed text-[var(--text-muted)]">
              {p}
            </p>
          ))}
          {reading.scriptureRefs && reading.scriptureRefs.length > 0 && (
            <footer className="flex flex-wrap gap-2 pt-1">
              {reading.scriptureRefs.map((ref) => (
                <span
                  key={ref}
                  className="rounded-full border border-[var(--accent)]/20 bg-[var(--accent-light)] px-2.5 py-0.5 text-[0.6875rem] font-medium text-[var(--accent)]"
                >
                  {ref}
                </span>
              ))}
